  ...
  (set, get) => ({
    ...
    addItem: (newItem) => {
      set((state) => {
        ...
        if (existing) {
          return { items: state.items.map((i) => i.product_id === newItem.product_id ? { ...i, quantity: i.quantity + newItem.quantity } : i) };
        }
        return { items: [...state.items, newItem] };
      });
      toast.success(newItem.name + " added to cart", { duration: 2000 });
      set({ isOpen: true });
    },
    ...
    subtotal: () => get().items.reduce((sum, i) => sum + parseFloat(String(i.price)) * i.quantity, 0),
  }),
  {
    name: "ayurveda-cart",
